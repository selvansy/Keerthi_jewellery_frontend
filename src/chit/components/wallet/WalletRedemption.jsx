...
function WalletRedemption() {
  ...
  const validateForm = () => {
    ...
    if (!formData.redeem_type && !isCustomer)
      errors.redeem_type = "ReedeemType is required";
      ...
      cell: (row) => {
        ...
        return (
          <div className="flex flex-col">
            <span>
              {row?.id_scheme?.scheme_name}
              {/* {row?.id_scheme?.code} */}
            </span>
            <span>
              {user?.id_scheme_account?.firstname &&
                user?.id_scheme_account?.lastname &&
                `${user?.id_scheme_account?.firstname || ""} ${
                  user?.id_scheme_account?.lastname || ""
                }`}
            </span>
  ...
  return (
    <>
      <div className="flex flex-row justify-between items-center w-full sm:order-1 sm:w-auto sm:mr-auto md:order-1 md:w-auto md:mr-auto ">
        <div className="w-1/2 sm:w-auto me-1 mt-2">
          <Breadcrumb
            items={[
              { label: "Wallet" },
              { label: "Wallet Redemption", active: true },
            ]}
          />
        </div>
      </div>
      ...
      <div className="w-full flex flex-col bg-white border-[1px] border-[#f2f3f8] rounded-md px-6 overflow-y-auto scrollbar-hide gap-x-8 p-5">
        <h2 className="text-md font-bold text-[#232323] mb-4">
          {id ? "Referral History" : "Add Wallet Redemption"}
        </h2>
        {!id && (
          <>
            <div className="flex flex-col mt-5 relative ">
              <label className="text-[#232323] mb-1 font-semibold text-sm ">
                ...
                <table className="min-w-[600px] table-auto my-3 w-full">
                  ...
                  <tbody>
                    <tr className="text-sm md:text-base">
                      ...
                      <td className="text-start px-4 py-2">
                        {walletData.active_scheme ?? "-"}
                      </td>
                      <td className="text-start px-4 py-2">
                        {walletData?.balance_amt !== undefined
                          ? `₹${spliceDecimals(walletData.balance_amt, 2)}`
                          : "-"}
                      </td>

                      <td className="text-start px-4 py-2">
                        {walletData?.redeem_amt !== undefined
                          ? `₹${spliceDecimals(walletData.redeem_amt, 2)}`
                          : "-"}
                      </td>
                    </tr>
                  </tbody>
          ...
          <>
            <div className="grid grid-col-2 md:grid-cols-3 gap-6 mt-8 md:mt-0">
              <div className="flex flex-col gap-2">
                <label className="text-[#232323] text-sm font-semibold">
                  Redeem Amount<span className="text-red-400"> *</span>
                </label>
                <input
                  type="text"
                  name="redeem_amt"
                  maxLength={"5"}
                  value={spliceDecimals(formData.redeem_amt,2)}
                  onInput={(e) =>
                    (e.target.value = e.target.value.replace(/\D/g, ""))
                  }
                  pattern="\d{5}"
              ...
              {(!isCustomer ||
                ...
                <>
                  <div className="flex flex-col gap-2">
                    ...
                    <Select
                      name="redeem_type"
                      value={redeem_type.find(
                        (option) => option.value === formData.redeem_type
                      )}
              ...
              {!isCustomer &&
                (formData.redeem_type === "1" ||
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <Select
                      name="paymentData"
                      value={paymentData.find(
                        (option) => option.value === formData.payment_mode
                      )}
                      onChange={(selectedOption) =>
        ...
        {id && (
          <>
            ...
            >
              <RefferalCusCard refData={cusdata} />
            </ModelOne>
          </>
        )}
      </div>
