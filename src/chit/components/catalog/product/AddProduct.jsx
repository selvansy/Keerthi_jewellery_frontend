...
useEffect(() => {
  ...
  const { mutate: getProdcutById } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setMakingCharge(response.data);
      setFormData(response.data);
      setExistingImages(response.data.product_image || []); // Set existing images
  ...
  const handleRemoveImage = (index) => {
    if (index < existingImages.length) {
      ...
      if (imageToRemove instanceof File) {
        const previewUrl = URL.createObjectURL(imageToRemove);
        URL.revokeObjectURL(previewUrl);
      }
  ...
  const validateFormData = () => {
    ...
    if (totalImages === 0) {
      errors.product_image = "At least one image is required";
    }
              ...
              <div className="flex gap-4 items-start">
                ...
                {totalImagesCount < MAX_IMAGES && (
                  <div className="flex items-center border border-[#F2F2F9] bg-white rounded h-[44px] overflow-hidden relative w-full max-w-xs">
                    ...
                    <div className="px-3 text-sm text-gray-500 w-full">
                      {totalImagesCount > 0
                        ? `${totalImagesCount} file(s) selected`
                        : "Browse"}
                    </div>
