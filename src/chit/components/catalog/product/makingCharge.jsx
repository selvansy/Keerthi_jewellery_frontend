...
export default function MakingChargesForm({ onChange, initialState }) {
  ...
  const handleActualValueChange = (e) => {
    ...
    if (value && formData.discountedValue) {
      const percentage = (
        ((Number(value) - Number(formData.discountedValue)) / Number(value)) *
        100
      ).toFixed(2);
  ...
  const handleDiscountedChange = (e) => {
    let value = e.target.value.replace(/[^0-9]/g, "");
    if (formData.actualValue && Number(value) > Number(formData.actualValue))
      return;
    ...
    if (formData.actualValue && value) {
      const percentage = (
        ((Number(formData.actualValue) - Number(value)) /
          Number(formData.actualValue)) *
        100
      ).toFixed(2);
      handleInputChange("discountedPercentage", percentage);
    } else {
        ...
        <div className="flex items-center  gap-6 mt-6 mb-2 w-full">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={() =>
                handleInputChange("discountView", !formData.discountView)
              }
              className="hidden"
              id="discountView"
            />
