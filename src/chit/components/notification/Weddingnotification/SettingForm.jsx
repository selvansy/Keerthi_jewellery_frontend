...
import { createweddingbirth,updateweddingbirth,getweddingbirthbyid,getnotificationtype } from '../../../api/Endpoints';
...
function SettingNotification({ setIsSettingOpen}) {
  ...
  const { mutate: handleweddingbirthbyid } = useMutation({
    ...
    const { mutate: handlegetnotificationtype } = useMutation({
  ...
  useEffect(() => {
    if (id) {
      handleweddingbirthbyid(id);
      handlegetnotificationtype();
      getweddingbirthbyid({type:1})
    }
