...
const OurEmployee = () => {
  ...
  // Fetch employees mutation
  const { mutate: fetchEmployees } = useMutation({
  ...
  }, [currentPage, itemsPerPage, debouncedSearch, id_branch]);
  ...
  const { mutate: deleteEmployeeMutate } = useMutation({
    ...
    onSuccess: (response) => {
      ...
      // Handle page reset if we deleted the last item on the page
  ...
  // Event listener setup for delete confirmation
  useEffect(() => {
    const handleDeleteConfirm = (data) => {
      deleteEmployeeMutate(data.employeeId);
    };

    eventEmitter.on("CONFIRMATION_SUBMIT", handleDeleteConfirm);

    return () => {
      eventEmitter.off("CONFIRMATION_SUBMIT", handleDeleteConfirm);
    };
  }, [employeeData.length, currentPage]);
